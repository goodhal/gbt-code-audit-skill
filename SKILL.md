...
3. **学习标准规则**
   ```bash
   # 获取标准规则（standard: 34943/34944/34946/39412）
...
for standard in standards:
    rules = get_rules(standard=standard)
    # 学习规则内容
```
...
```
工具扫描发现 → 查找对应 CWE → 检查是否在已学习的国标规则中 → 决定处理方式
```
...
| 工具名称 | 描述 | 命令行用法 |
|---------|------|----------|
| `detect_language` | 检测代码目录使用的语言，返回语言列表和对应的标准 | `python skill.py detect_language <target>` |
| `get_standards` | 获取语言对应的审计标准 | `python skill.py get_standards --languages=java,python` |
| `get_rules` | 获取标准的完整规则列表（包含审计要点和修复建议） | `python skill.py get_rules <standard>` |
| `scan` | 执行工具扫描（SpotBugs 字节码扫描） | `python skill.py scan <target> --bytecode` |
| `get_report_template` | 返回标准的报告模板，供生成报告时参考 | `python skill.py get_report_template` |
| `audit_code` | 使用 LLM 对代码进行安全审计 | `python skill.py audit_code <target>` |

**命令行参数说明**：
- `get_rules <standard>`：standard 为标准号（34943/34944/34946/39412），返回完整规则内容
- 示例：`python skill.py get_rules 39412`
